    ...
    <>
      <form ref={formRef} onSubmit={handleSubscribe} className="relative flex">
        <input
          ...
          type="email"
          className="w-full rounded-lg border border-transparent bg-neutral p-2.5 outline-none placeholder:text-fg-neutral-faded focus:border-border-neutral-faded focus:bg-neutral focus:ring-border-neutral-faded"
        />
...
const Modal = ({ open, onOpenChange, modalText }: any) => (
  <Dialog.Root open={open} onOpenChange={onOpenChange}>
    <Dialog.Portal>
      <Dialog.Overlay className="bg-black/25 fixed inset-0 backdrop-blur-xl" />
      <Dialog.Content className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 space-y-4 rounded-xl bg-elevated p-8">
        <Dialog.Title className="text-xl font-semibold">{modalText.title}</Dialog.Title>
